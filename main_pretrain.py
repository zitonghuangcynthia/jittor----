...
#学习图像的共性特征，使得来自同一图像的不同视角或裁剪之间的特征在特征空间中更加一致。
import math
import os
import shutil
import time
...
def main():
    ...
    # optionally resume from a checkpoint，从之前的checkpoint继续开始
    to_restore = {"epoch": 0}
    restart_from_checkpoint(
...
def train(train_loader, model, optimizer, epoch, lr_schedule, queue):
    batch_time = AverageMeter() #用于计算和记录训练过程中的时间和损失值的平均数的工具
...
def swav_loss(args, model, embedding, output, queue, use_the_queue, bs):    #鼓励学习图像的共性特征
    embedding = embedding.detach()  #获取输入图像的特征向量embedding
    labels = []
    # ============ swav loss ... ============
    loss = 0
    for i, crop_id in enumerate(args.crops_for_assign): #crop_id是从图像中不同位置裁剪得到的子图像
        with jt.no_grad():
            out = output[bs * crop_id : bs * (crop_id + 1)].detach()    #对每个自图像都需要输出一个特征
