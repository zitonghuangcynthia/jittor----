...
class ResNet(nn.Module):
    def __init__(self,
        ...
        '''
        需要load sam backbone部分的pretrain权重
        '''

        pretrain_weight_path = "/home/hzt/pass-jittor/weights/pass50/sam_vit_b_01ec64.pth"
        pretrain_weights = jt.load(pretrain_weight_path)
        filtered_parameters = {k: v for k, v in pretrain_weights.items() if k in self.image_encoder.state_dict()} 
        self.image_encoder.load_parameters(filtered_parameters)
        #self.image_encoder.load_parameters(jt.load(sam_weight_path))
        ...
        self.conv1x1_3 = nn.Conv2d(768, 512, 1)
        self.conv1x1_2 = nn.Conv2d(768, 256, 1)
        self.conv1x1_1 = nn.Conv2d(768, 128, 1)
        self.conv1x1_0 = nn.Conv2d(768, 64, 1)
        num_out_filters *= 8
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
    ...
    def execute_backbone(self, x, avgpool=True):
        ...
        if self.image_encoder.pos_embed is not None:
            if x.shape[-2] != 14:
                x = x + self.image_encoder.pos_embed_1
            else:
        ...
        x = self.conv1x1_3(features[3])
        c3 = self.conv1x1_2(features[2])
        c2 = self.conv1x1_1(features[1])
        c1 = self.conv1x1_0(features[0])
        return x, c3, c2, c1
